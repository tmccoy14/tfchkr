...
VERSION = "0.1.0"
CONTEXT_SETTINGS = dict(auto_envvar_prefix="tfstate")
HOME = str(Path.home())
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.secho("tfstate version: v%s" % VERSION, fg="blue")
    ctx.exit()
...
class TFSTATE_CLI(click.MultiCommand):
...
@click.command(cls=TFSTATE_CLI, context_settings=CONTEXT_SETTINGS)
@click.option(
    "--home",
...
@click.option(
    ...
    expose_value=False,
    is_eager=True,
    help="Print the current version of tfstate.",
)
@pass_environment
def cli(ctx, verbose, home):
    """
    tfstate is a tool to run tests on Terraform modules and files.
    """
    ctx.verbose = verbose
    if home is not None:
        ctx.home = home
